...
document.addEventListener("DOMContentLoaded", function() {
    ...
    chatInput.addEventListener("copy", function(event) {
        event.preventDefault();
...
function movePlayer(playerId, team) {
    const teamCodes = { red: 1, blue: 2, spectator: 0 };
    const targetTeam = teamCodes[team.toLowerCase()];
    if (targetTeam !== undefined) {
        room.setPlayerTeam(playerId, targetTeam);
    } else {
        console.error("Equipo no válido:", team);
    }
}
...
<button onclick="toggleSettingsMenu()">⚙ Configuración</button>
<div id="settingsMenu" style="display:none;">
    <label>Zoom de Cámara:
        <input type="range" min="0.5" max="2" step="0.1" onchange="setCameraZoom(this.value)">
    </label>
...
<button onclick="showReplays()">📂 Ver Repeticiones</button>
<div id="replayMenu" style="display:none;">
    <ul id="replayList">
        <!-- Las repeticiones se cargarán aquí -->
    </ul>
</div>

<script>
function showReplays() {
    const menu = document.getElementById("replayMenu");
    menu.style.display = "block";
    const replayList = document.getElementById("replayList");
    // Supongamos que tienes una función que obtiene las repeticiones disponibles
    const replays = getAvailableReplays();
    replayList.innerHTML = "";
    replays.forEach(function(replay) {
        const listItem = document.createElement("li");
        listItem.textContent = replay.name;
...
function toggleRecording() {
    ...
    if (recording) {
        ...
        // Aquí puedes implementar la lógica para guardar recordedData
    }
}

// Supongamos que esta función se llama cada vez que hay un evento en el juego
